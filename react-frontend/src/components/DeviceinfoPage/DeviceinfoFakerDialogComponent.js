...
import deviceinfoFakerFactory from "./deviceinfoFakerFactory";
...
const DeviceinfoFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = deviceinfoFakerFactory(fakerCount);
        let results = [];
        setLoading(true);
        try {
            for (let i = 0; i < fakeData.length; i++) {
                const _data = fakeData[i];
                const res = await client.service("deviceinfo").create(_data);
                // const res = _data;
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(DeviceinfoFakerDialogComponent);
